...
\caption{Lens plane, both the surface density $\sigma$ (left) and the lensing potential $\phi$ (right) are shown.}
\label{lensplanefig}
\end{figure*}
%
...
\caption{Directory tree structure and corresponding class inheritance in the \LT ray--tracing operations}
\label{inheritance}
\end{figure*}
...
\caption{Peak memory usage for the lens plane generation (black) and ray--tracing (red) for the test case indicated in Table \ref{benchmarktable}}
\label{memoryfig}
\end{figure}
