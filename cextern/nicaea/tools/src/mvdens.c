...
/* L has to be a (ndim x ndim) trianglular matrix (e.g. after Cholesky-decomposition) */
...
double ln_determinant(const double *L, size_t ndim, error **err)
{
   size_t i;
   double ln_det;

   for (i=0,ln_det=0.0; i<ndim*ndim; i+=(ndim+1)) {
      testErrorRetVA(L[i] <= 0, mv_negative, "Negative diagonal entry L_{%d,%d} = %g, ln det not defined",
		     *err, __LINE__, 0.0, i/(ndim+1), i/(ndim+1), L[i]);
      ln_det += log(L[i]);
  }
